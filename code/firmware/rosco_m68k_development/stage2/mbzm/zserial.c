  ...
  if (IS_ERROR(result)) {
    ...
    if (in_32bit_block) {
      ...
      if (recv_crc == calc_crc) {
        DEBUGF("  >> READ_BLOCK: CRC32 is good (recv: 0x%08lx; calc: 0x%08lx)\n", recv_crc, calc_crc);
        return result;
      } else {
        DEBUGF("  >> READ_BLOCK: CRC32 is borked (recv: 0x%08lx; calc: 0x%08lx)\n", recv_crc, calc_crc);
        return BAD_CRC;
      }
    } else {
...
ZRESULT zm_read_binary32_header(ZHDR *hdr) {
  ...
  for (int i = 0; i < ZHDR_SIZE; i++) {
    ...
    if (IS_ERROR(b)) {
      ...
      *ptr++ = (uint8_t)b;
      ...
      DEBUGF("READ_BIN32: CRC after byte %d is 0x%08lx\n", i, crc);
    }
  }
  ...
  DEBUGF("READ_BIN32: All read; check CRC (Received: 0x%08lx; Computed: 0x%08lx)\n",
