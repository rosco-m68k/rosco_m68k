...
{
    while (!HD_FAT_initialize())
    {
        printf("\nNo HDD detected. SPACE to retry, other key to warm-boot: ");
        char key = readchar();
        if (key != ' ')
        {
            printf("exit\n");
...
void command_prompt()
{
    ...
    printf("\nrosco_m68k: IDE HDD nano-shell prompt (built " __DATE__ " " __TIME__ ")\n\n");
    while (true)
    {
        //check_sd_card();
        printf("%s> ", fullpath(""));
        ...
        case CMD_EXIT:
            ...
            return;
        default:
            printf("IDE HDD prompt commands:\n");
            for (cmd_num = 0; cmd_num < NUM_CMD; cmd_num++)
            {
                const char * cmd_arg =
...
#endif
    printf("\nrosco_m68k [FW:%X.%02X%s]: IDE HDD Menu - (c) 2020 Xark, MIT License\n",
