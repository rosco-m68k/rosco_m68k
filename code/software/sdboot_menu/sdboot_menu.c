...
#define MAX_BIN_NAMELEN 255                        // full name length
...
{
    static char filename[MAX_BIN_NAMELEN];        // temp filename string buffer
    ...
    {
        snprintf(filename, sizeof(filename) - 1, "/sd/%s%s%s", current_dir, strlen(current_dir) > 0 ? "/" : "", path);
    }
    ...
    if ((dir = opendir(fullpath(""))) != NULL)
    {
        ...
        while ((dent = readdir(dir)) != NULL)
        {
            ...
            if (!dent->d_is_dir)
            {
                ...
                if (len >= 4)
                {
                    const char * ext = dent->d_name + len - 4;
                    if (strcasecmp(ext, ".bin") == 0 || strcasecmp(ext, ".txt") == 0)
                    {
                        if (num_menu_files < MAX_BIN_FILES)
                        {
                            strncpy(menu_files[num_menu_files], dent->d_name, MAX_BIN_NAMELEN - 1);
                            bin_sizes[num_menu_files] = dent->d_size;
                            num_menu_files++;
                        }
                        else
                        {
            ...
            else if (strcmp(dent->d_name, ".") != 0)
            {
                ...
                {
                    strncpy(dir_files[num_dir_files], dent->d_name, MAX_BIN_NAMELEN - 1);
                    num_dir_files++;
                }
                else
                {
...
static void execute_bin_file(const char * name, uint8_t * loadstartptr)
{
    ...
    if (file != NULL)
    {
        ...
        while (loadptr < endptr && (c = fread(loadptr, 1, 512, file)) > 0)
        {
            loadptr += c;
...
static void file_operation(const char * name, int (*op_func)(char * p, int l))
{
    ...
    {
        int c;
        while ((c = fread(buffer, 1, sizeof(buffer), file)) > 0)
        {
            ...
            case CMD_WRITE: {
                ...
                if (file != NULL)
                {
                    ...
                    while (filesize < (512 * 1024))
                    {
                        ...
                        int l = fwrite(buffer, 1, sizeof(buffer), file);
                        if (l != sizeof(buffer))
                        {
        ...
        do
        {
            ...
            if (rawkey == 'k')
            {
                ...
                for (int retry = 0; retry < 10000; retry++)
                {
                    ...
                    {
                        rawkey = mcInputchar();
                        if (rawkey == '\x01' || rawkey == '\x11')        // ^A upload key
                        {
                            key = rawkey;
                            break;
                        }
