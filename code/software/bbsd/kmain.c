...
uint32_t crc32b(const void * buf, size_t size)
{
  const uint8_t * p = buf;
  uint32_t crc;
        ...
        if (init_status == BBSD_INIT_OK) {
            ...
            if (BBSD_make_device(&sd, &block_device)) {
                ...
                if (file != NULL) {
                    ...
                    timer_start();
                    while ((c = fl_fread(loadptr, 512, 1, file)) > 0) {
                        loadptr += c;
                        /* period every 16KiB, does not noticeably affect speed */
                        if ((((uint16_t)((uint32_t)loadptr)) & 0x3fff) == 0)
                            mcSendchar('.');
                    }
